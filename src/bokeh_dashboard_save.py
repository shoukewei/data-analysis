...
def fetch_github_data(repos=None):
    ...
    days_since = (datetime.now(timezone.utc) - df["updated_at"]).dt.days
...
p_ts.scatter("created_at", "cumulative", source=ts_source,
...
def update(attr, old, new):
    ...
    source.data = dict(ColumnDataSource(filt).data)
...
# More reliable check: only skip if we are actually running via "bokeh serve"
running_with_bokeh_serve = any("bokeh" in arg.lower() and "serve" in arg.lower() 
                               for arg in sys.argv)

if not running_with_bokeh_serve:
    out_path = "../outputs/bokeh_dashboard.html"
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    
    try:
        from bokeh.io import output_file, save
        
        output_file(out_path, title="GitHub Repository Dashboard")
        save(dashboard, filename=out_path)
        
        print(f"✅ Successfully saved static dashboard to:")
        print(f"   {os.path.abspath(out_path)}")
        print(f"   Open this HTML file in your browser to view it.")
    ...
    print("Running with 'bokeh serve' — skipping static HTML export (normal behavior).")